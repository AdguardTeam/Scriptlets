...
(function () {
     ...
     */
    function getWildcardPropertyInChain(base, chain, lookThrough = false, output = []) {
        const pos = chain.indexOf('.');
        if (pos === -1) {
     ...
     * @returns {number|null}
     */
    const getNumberFromString = (rawString) => {
     ...
     * @returns {string}
     */
    const replaceAll = (input, substr, newSubstr) => input.split(substr).join(newSubstr);
     ...
     * @returns {boolean}
     */
    const validateStrPattern = (input) => {
        const FORWARD_SLASH = '/';
        let str = input;
        if (input[0] === FORWARD_SLASH && input[input.length - 1] === FORWARD_SLASH) {
            str = input.slice(1, -1);
    ...
    const substringAfter$1 = (str, separator) => {
        if (!str) {
            return str;
        }
    ...
    const substringBefore = (str, separator) => {
        if (!str || !separator) {
            return str;
        }
        const index = str.indexOf(separator);
    ...
    */
    const convertRtcConfigToString = (config) => {
     ...
     */
    const noopPromiseReject = () => Promise.reject(); // eslint-disable-line compat/compat

    /**
     * Returns Promise object that is resolved with a response
     * @param {string} [responseBody='{}'] value of response body
     */
    const noopPromiseResolve = (responseBody = '{}') => {
     ...
     */
    const observeDOMChanges = (callback, observeAttrs = false, attrsToObserve = []) => {
        /**
     ...
     * @returns {boolean}
     */
    const matchStackTrace = (stackMatch, stackTrace) => {
     ...
     */
    const prepareCookie = (name, value) => {
        if (!name || !value) {
            return null;
        }
     ...
     * @returns {boolean}
     */
    const isDelayMatched = (inputDelay, realDelay) => {
     ...
     */
    const getBoostMultiplier = (boost) => {
     ...
     */
    const parseMatchProps = (propsToMatchStr) => {
        const PROPS_DIVIDER = ' ';
        const PAIRS_MARKER = ':';
    ...
    const handleOldReplacement = (replacement) => {
        let result;
    ...
    const getPreventGetter = (nativeGetter) => {
     ...
     * @returns {string}
     */
    const listenerToString = (listener) => {
    ...
    abortOnPropertyRead$1.injections = [
        randomId,
        setPropertyAccess,
        getPropertyInChain,
    ...
    abortOnPropertyWrite$1.injections = [
        randomId,
        setPropertyAccess,
        getPropertyInChain,
        createOnErrorHandler,
    ...
    preventSetTimeout$1.injections = [
        hit,
        noopFunc,
        parseMatchArg,
        parseDelayArg,
    ...
    preventSetInterval$1.injections = [
        hit,
        noopFunc,
        parseMatchArg,
        parseDelayArg,
     ...
     */
    /* eslint-enable max-len */
    function preventWindowOpen$1(source, match = getWildcardSymbol(), delay, replacement) {
    ...
    preventWindowOpen$1.injections = [
        hit,
        validateStrPattern,
        validateMatchStr,
        toRegExp,
    ...
    abortCurrentInlineScript$1.names = [
    ...
    abortCurrentInlineScript$1.injections = [
        randomId,
        setPropertyAccess,
        getPropertyInChain,
        toRegExp,
    ...
    preventAddEventListener$1.names = [
    ...
    preventAddEventListener$1.injections = [
        hit,
        toRegExp,
        validateType,
        validateListener,
...
listener: ${convertTypeToString(listener)}`;
    ...
    logAddEventListener$1.injections = [
        hit,
        validateType,
        validateListener,
        listenerToString,
    ...
    debugOnPropertyRead$1.names = [
        'debug-on-property-read',
    ];
    debugOnPropertyRead$1.injections = [
        randomId,
        setPropertyAccess,
        getPropertyInChain,
        createOnErrorHandler,
    ...
    debugOnPropertyWrite$1.names = [
        'debug-on-property-write',
    ];

    debugOnPropertyWrite$1.injections = [
        randomId,
        setPropertyAccess,
        getPropertyInChain,
        createOnErrorHandler,
    ...
    debugCurrentInlineScript$1.names = [
        'debug-current-inline-script',
    ];

    debugCurrentInlineScript$1.injections = [
        randomId,
        setPropertyAccess,
        getPropertyInChain,
        toRegExp,
     ...
     */
    /* eslint-enable max-len */
    function removeAttr$1(source, attrs, selector, applying = 'asap stay') {
        if (!attrs) {
            return;
        }
     ...
     */
    /* eslint-enable max-len */
    function setAttr$1(source, selector, attr, value = '') {
        if (!selector || !attr) {
            return;
        }
    ...
    function removeClass$1(source, classNames, selector, applying = 'asap stay') {
        if (!classNames) {
            return;
        }
    ...
    disableNewtabLinks$1.injections = [
        hit,
    ];
    ...
    adjustSetInterval$1.injections = [
        hit,
        toRegExp,
        getBoostMultiplier,
        isDelayMatched,
    ...
    preventRequestAnimationFrame$1.names = [
    ...
    preventRequestAnimationFrame$1.injections = [
        hit,
        noopFunc,
        parseMatchArg,
        validateStrPattern,
    ...
    hideInShadowDom$1.names = [
        'hide-in-shadow-dom',
    ];

    hideInShadowDom$1.injections = [
        hit,
        observeDOMChanges,
        flatten,
        findHostElements,
    ...
    removeInShadowDom$1.names = [
        'remove-in-shadow-dom',
    ];

    removeInShadowDom$1.injections = [
        hit,
        observeDOMChanges,
        flatten,
        findHostElements,
     ...
     */
    /* eslint-enable max-len */
    function preventFetch$1(source, propsToMatch, responseBody = 'emptyObj') {
    ...
    abortOnStackTrace$1.injections = [
        randomId,
        setPropertyAccess,
        getPropertyInChain,
        createOnErrorHandler,
    ...
    logOnStacktrace$1.names = [
        'log-on-stack-trace',
    ];
    logOnStacktrace$1.injections = [
        getPropertyInChain,
        setPropertyAccess,
        hit,
    ];
     ...
     * ```
     */
    function forceWindowClose$1(source, path = '') {
        // eslint-disable-next-line no-console
        const log = console.log.bind(console);
    ...
    preventElementSrcLoading$1.names = [
        'prevent-element-src-loading',
    ];

    preventElementSrcLoading$1.injections = [
        hit,
        toRegExp,
        safeGetDescriptor,
    ];
    ...
    const validAdgRedirects = redirects$1.filter((el) => el.adg);
     ...
     * @returns {Array}
     */
    const parseModifiers = (rule) => substringAfter$1(rule, '$').split(',');
    ...
    */
     ...
     */
    const convertUboScriptletToAdg = (rule) => {
     ...
     */
    const convertAbpSnippetToAdg = (rule) => {
     ...
     */
    const convertScriptletToAdg = (rule) => {
        let result;
     ...
     * @returns {boolean}
     */
    const isValidScriptletRule = (input) => {
        if (!input) {
            return false;
        }
     ...
     * @returns {string}
     */
    const convertUboRedirectToAdg = (rule) => {
     ...
     * @returns {string}
     */
    const convertAbpRedirectToAdg = (rule) => {
        const firstPartOfRule = substringBefore(rule, '$');
        ...
        const adgModifiers = abpModifiers
            .map((modifier) => {
                if (modifier.indexOf(validator.REDIRECT_RULE_TYPES.ABP.redirectMarker) > -1) {
                    const abpName = substringAfter$1(
                        modifier,
     ...
     * @returns {string}
     */
    const convertRedirectToAdg = (rule) => {
        let result;
     ...
     * @returns {string}
     */
    const convertAdgRedirectToUbo = (rule) => {
    ...
    function jsonPrune(source, propsToRemove, requiredInitialProps, stack) {
      ...
      function isPruningNeeded(root) {
        ...
        for (var i = 0; i < requiredPaths.length; i += 1) {
          ...
          for (var _i2 = 0; _i2 < details.length; _i2 += 1) {
            if (hasWildcard) {
              // if there is a wildcard,
              // at least one (||) of props chain should be present in object
              shouldProcess = !(details[_i2].base[lastNestedPropName] === undefined) || shouldProcess;
            } else {
              // otherwise each one (&&) of them should be there
              shouldProcess = !(details[_i2].base[lastNestedPropName] === undefined) && shouldProcess;
            }
    ...
    function preventRequestAnimationFrame(source, match) {
      ...
      var _parseMatchArg4 = parseMatchArg(match),
          isInvertedMatch = _parseMatchArg4.isInvertedMatch,
          matchRegexp = _parseMatchArg4.matchRegexp;
    ...
    function preventSetInterval(source, match, delay) {
      ...
      var _parseMatchArg2 = parseMatchArg(match),
          isInvertedMatch = _parseMatchArg2.isInvertedMatch,
          matchRegexp = _parseMatchArg2.matchRegexp;

      var _parseDelayArg2 = parseDelayArg(delay),
          isInvertedDelayMatch = _parseDelayArg2.isInvertedDelayMatch,
          delayMatch = _parseDelayArg2.delayMatch;
    ...
    function preventWindowOpen(source) {
      ...
      var newOpenWrapper = function newOpenWrapper(url) {
        ...
        if (match === getWildcardSymbol()) {
          ...
          var _parseMatchArg3 = parseMatchArg(match),
              isInvertedMatch = _parseMatchArg3.isInvertedMatch,
              matchRegexp = _parseMatchArg3.matchRegexp;
    ...
    function removeInShadowDom(source, selector, baseSelector) {
      ...
      var removeHandler = function removeHandler() {
        ...
        while (hostElements.length !== 0) {
          ...
          var _pierceShadowDom2 = pierceShadowDom(selector, hostElements),
              targets = _pierceShadowDom2.targets,
              innerHosts = _pierceShadowDom2.innerHosts;
    ...
    function setSessionStorageItem(source, key, value) {
      ...
      var setItem = function setItem(key, value) {
        var _window2 = window,
            sessionStorage = _window2.sessionStorage; // setItem() may throw an exception if the storage is full.
